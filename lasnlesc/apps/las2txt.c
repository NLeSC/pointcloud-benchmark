...
static uint64_t morton2D_encode(unsigned int x, unsigned int y){
    uint64_t answer = 0;
    uint64_t i;
    for (i = 0; i < (sizeof(uint64_t)* CHAR_BIT)/2; ++i) {
        answer |= ((y & ((uint64_t)1 << i)) << i) | ((x & ((uint64_t)1 << i)) << (i + 1));
    }
    return answer;
}
   ...
   /* scaled offsets to add for the morton encoding */
    ...
    while (p)
    {
        ...
        for (;;)
        {
            ...
            case 'k':
                rawx = ((unsigned int) LASPoint_GetRawX(p)) + factorX;
                rawy = ((unsigned int) LASPoint_GetRawY(p)) + factorY;
                mortonkey = morton2D_encode(rawx,rawy);
                fprintf(file_out, "%"PRIu64, mortonkey);
                break;
            /* // the x coordinate */      
            case 'x': 
